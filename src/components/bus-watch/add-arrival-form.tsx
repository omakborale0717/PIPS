...
type AddArrivalFormProps = {
    onArrivalAdded: (newArrival: Arrival) => void;
}

export default function AddArrivalForm({ onArrivalAdded }: AddArrivalFormProps) {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    setIsLoading(true);
    const result = await addArrival(values);
    setIsLoading(false);

    if (result.success) {
        toast({
            title: "Arrival Added",
            description: `Successfully added arrival for route ${values.route}.`
        });
        form.reset();
        onArrivalAdded(result.data);
    } else {
        toast({
            variant: "destructive",
            title: "Error",
